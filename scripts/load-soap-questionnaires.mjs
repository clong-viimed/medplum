#!/usr/bin/env node
// Load SOAP section Questionnaire resources into Medplum
...
const DATA_DIR = '../../medplum-provider/src/data/soap-questionnaires';
...
async function main() {
  ...
  console.log(`Loading ${jsonFiles.length} SOAP Questionnaire resource(s)...`);
...
async function createMedplumClientFromEnv() {
  ...
  throw new Error(
    'Set MEDPLUM_ACCESS_TOKEN, MEDPLUM_CLIENT_ID/MEDPLUM_CLIENT_SECRET, or MEDPLUM_EMAIL/MEDPLUM_PASSWORD before running.\n' +
      'Example: MEDPLUM_EMAIL=admin@example.com MEDPLUM_PASSWORD=medplum_admin node scripts/load-soap-questionnaires.mjs'
  );
}
...
function printHelp() {
  console.log(`Usage: node scripts/load-soap-questionnaires.mjs

Loads the SOAP section Questionnaire resources into Medplum.
...
Example:
  MEDPLUM_EMAIL=admin@example.com MEDPLUM_PASSWORD=medplum_admin node scripts/load-soap-questionnaires.mjs
`);
}
